...
test('Sign up and verify via email', async ({ page, context }) => {
  ...
  await page.getByRole('textbox', { name: 'Password', exact: true }).fill(process.env.PASS);
  await page.getByRole('textbox', { name: 'Confirm Password' }).click();
  await page.getByRole('textbox', { name: 'Confirm Password' }).fill(process.env.PASS);
  ...
  await page.getByRole('textbox', { name: 'OTP input 4' }).fill('4');
